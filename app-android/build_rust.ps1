...
$jniOut = Resolve-Path (Join-Path $PSScriptRoot 'android/app/src/main/jniLibs')

$args = @('ndk', '-t', 'arm64-v8a', '-o', $jniOut.Path, 'build', '-p', 'app-android')
...
$so = Join-Path $jniOut.Path 'arm64-v8a/libapp_android.so'
Write-Host "Built: $so"

...
echo "==> cargo build -p bridge-ios --target $TARGET --release"
( cd "$REPO_ROOT" && cargo build -p bridge-ios --target "$TARGET" --release )

STATICLIB="$REPO_ROOT/target/$TARGET/release/librustplayer.a"

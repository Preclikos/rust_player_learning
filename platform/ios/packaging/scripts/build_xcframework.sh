...
PKG_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"          # platform/ios/packaging
REPO_ROOT="$(cd "$PKG_DIR/../../.." && pwd)"      # repo root (3 up)
...
build_lib() {
    ...
    local staticlib="$REPO_ROOT/target/$rust_target/release/librustplayer.a"
